...
function ShoppingCart({ cart, setCart, user, setUser, setLoaded }) {
	...
	const [showEmptyCartMobile, setShowEmptyCartMobile] = useState(false);
	useEffect(() => {
		const setResponsiveness = () => {
			if (window.innerWidth >= 580) return setShowEmptyCartMobile(true);
			else return setShowEmptyCartMobile(false);
		};
	...
	return (
		<>
			...
			<div
				...
				// 	width: "100%",
				// 	margin: "auto",
				// 	// height: `${ cart.length < 1 ? "35rem" : "auto"}`,
				// }}
			>
				{cart.length ? (
				...
				>
					...
					{cart.length ? (
						...
						<div style={{height:"30rem",display:"flex",flexDirection:"column",justifyContent:"center" }}>
							<img
...
/*flex-child1 shadow bg-white rounded p-200
	container shadow bg-white rounded p-3
 */
